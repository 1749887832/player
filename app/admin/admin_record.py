        ...
        for key, value in player_season[0].items():
            if key not in come_in:
                ...
                if key in ['hit_shoot', 'hit_throw', 'hit_points']:
                    left_count = 0.0 if left_data[-1]['value'] == 0.0 else round(left_sum / left_data[-1]['value'] * 100, 2)
                    right_count = 0.0 if right_data[-1]['value'] == 0.0 else round(right_sum / right_data[-1]['value'] * 100, 2)
